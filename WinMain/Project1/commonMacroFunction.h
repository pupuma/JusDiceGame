...
inline void DrawObject(HDC hdc, const RECT& rc, int size, const COLORREF penColor, const COLORREF brColor, const OBJ_TYPE nType, int rWidth = 10, int rHeight = 10, const int ropCode = R2_XORPEN)
{
	HPEN hPen, hOldPen;
	...
	case RECTANGLE:
		...
		break;
	case ROUNDRECT:
		RoundRect(hdc, rc.left, rc.top, rc.right, rc.bottom, rWidth, rHeight);
	}
...
inline void DrawObject(HDC hdc, const RECT& rc, int size, const COLORREF color, const OBJ_TYPE nType, int rWidth = 10, int rHeight = 10, const int ropCode = R2_XORPEN)
{
	DrawObject(hdc, rc, size, color, color, nType,rWidth, rHeight ,ropCode);
}
